...
QUnit.test('Collisions', function(assert) {
	...
	assert.ok(!s1.hits(s2), 'still colliding after 2 moves');

	s1.flight().move(vMove);
	assert.equal(s1.points()[0].x, -5, 'after 3 moves'); 
	assert.ok(!s1.hits(s2), 'still colliding after 3 moves');

	s1.flight().move(vMove);
	assert.equal(s1.points()[0].x, -10, 'after 4 moves'); 
	assert.ok(!s1.hits(s2), 'still colliding after 4 moves');
